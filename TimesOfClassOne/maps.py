...
class GameMap:
    ...
    def __init__(self, map_info: Dict[str, Any], loader: "loader"):
        self.name = map_info.get("Name", "Unnamed Map")
        self.width = map_info.get("Width", 20)
        self.height = map_info.get("Height", 20)
        # "宽度"对应x轴，"高度"对应y轴，矩阵用列优先表示, 即matrix[x][y]表示(x,y)位置的实体, 这样方便根据坐标获取实体
        # 地图下标从1开始，0留作无效值
        self.matrix: List[List[Optional[GameObject]]] = [[None for _ in range(self.height + 1)] for _ in range(self.width + 1)]
        # 预留接口，方便根据地图信息生成单位和建筑物实例
        self.loader = loader # 引用loader，方便根据地图信息生成单位和建筑物实例
    ...
    def remove_entity(self, entity: GameObject):
        ...
        entity.y = 0
# 这里需要del吗?
# 答:不需要del，因为在Python中，垃圾回收机制会自动处理不再使用的对象。当我们从地图上移除一个实体时，我们只是将地图矩阵中对应位置的引用设置为None，这样就断开了与该实体的引用关系。如果没有其他地方引用这个实体对象，那么它就会被垃圾回收器自动清理掉。因此，我们不需要显式地调用del来删除这个实体对象。

    def get_entity_at(self, x: Any, y: Optional[int] = None) -> Optional[GameObject]:
        """获取指定位置的实体"""
        if y is None:
            x, y = x
    ...
    def calc_range_entity_positions(self, entity: GameObject, range_info: Dict[str, Any]) -> List[Tuple[int, int]]:
        ...
        for pos in range_positions:
            if self.get_entity_at(pos) is not None:
    ...
    def calc_range_empty_positions(self, entity: GameObject, range_info: Dict[str, Any], ignore_obstacles: bool = False) -> List[Tuple[int, int]]:
        ...
        if ignore_obstacles:
            range_positions = self.calc_range_positions(entity, range_info)
            for pos in range_positions:
                if self.get_entity_at(pos) is None:
                ...
                for dx, dy in directions:
                    for dist in range(1, rmax + 1):
                        ...
                        if self.out_of_bounds(nx, ny):
                            break
                        if self.get_entity_at(nx, ny) is not None:
                            break # 遇到障碍物停止
